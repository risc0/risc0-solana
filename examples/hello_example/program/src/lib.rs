...
impl StoredVerificationKey {
    fn to_verification_key(&self) -> VerificationKey {
        VerificationKey {
...
impl Instruction {
    fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        match input[0] {
            0 => Ok(Instruction::InitializeVerifyingKey),
...
pub fn process_instruction(
    ...
    match instruction {
        Instruction::InitializeVerifyingKey => initialize_verifying_key(accounts),
...
fn verify_proof(accounts: &[AccountInfo], proof_data: &[u8]) -> ProgramResult {
    ...
    let stored_vk: StoredVerificationKey = borsh::from_slice(&vk_account.data.borrow())?;
    let vk = stored_vk.to_verification_key();
...
fn initialize_verifying_key(accounts: &[AccountInfo]) -> ProgramResult {
    let vk_account = &accounts[0];
    ...
    let verifying_key = StoredVerificationKey {
        nr_pubinputs: 81,
        vk_alpha_g1: [
        ...
        vk_delta_g2: [
            ...
            23, 154, 18, 217, 34, 219, 160, 84, 112, 87, 204, 170, 233, 75, 157, 105, 207, 170, 78,
            96, 64, 31, 234, 127, 62, 3, 51, 17, 12, 16, 19, 79, 32, 11, 25, 246, 73, 8, 70, 213,
            24, 201, 174, 168, 104, 54, 110, 251, 114, 40, 202, 92, 145, 210, 148, 13, 3, 7, 98,
            30, 96, 243, 31, 203, 247, 87, 232, 55, 232, 103, 23, 131, 24, 131, 45, 11, 45, 116,
            213, 158, 47, 234, 28, 113, 66, 223, 24, 125, 63, 198, 211,
        ],
        vk_ic: vec![
            [
    ...
    let serialized_vk = borsh::to_vec(&verifying_key)?;
    vk_account.data.borrow_mut().copy_from_slice(&serialized_vk);
...
mod tests {
    ...
    fn test_process_instruction() {
        ...
        // Create a mock account for storing the verification key
        let vk_key = Pubkey::new_unique();
        let mut lamports = 0;
        let mut data = vec![0u8; 840];
        ...
        let init_instruction = vec![0]; // 0 is the instruction code for InitializeVerifyingKey
        process_instruction(&program_id, &[vk_account.clone()], &init_instruction)
            .expect("Failed to initialize verifying key");

        // Load the proof data
        let proof_str = include_str!("../../../../test/data/r0_test_proof.json");
        let mut proof: Proof = serde_json::from_str(proof_str).unwrap();
        proof.pi_a = non_solana::negate_g1(&proof.pi_a).unwrap();
        ...
        // Verify the proof
        let result = process_instruction(&program_id, &[vk_account], &compressed_proof);
    ...
    fn test_public_inputs() {
        ...
        let receipt: Receipt = serde_json::from_slice(&receipt_json_str.as_slice()).unwrap();
