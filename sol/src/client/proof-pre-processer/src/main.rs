...
fn main() {
    process_and_write_lp_proof();
}

fn process_and_write_lp_proof() {
    let proof_a: [u8; 64] = PROOF[0..64]
        .try_into()
        .expect("Slice with incorrect length");
    let proof_a: [u8; 64] = preprocess_proof_a(&proof_a);
    ...
    let compressed_proof_a = compress_g1_be(&proof_a);
    let compressed_proof_b = compress_g2_be(&proof_b);
    let compressed_proof_c = compress_g1_be(&proof_c);

    write_to_file(
        "../lp_compressed_proof.bin",
        &compressed_proof_a,
        &compressed_proof_b,
        &compressed_proof_c,
    );
    write_proof_to_file("../lp_raw_proof.bin", &PROOF);

    println!("Compressed proof written to lp_compressed_proof.bin");
    println!("Proof written to lp_raw_proof.bin");
}

fn preprocess_proof_a(proof: &[u8]) -> [u8; 64] {
    let proof_a: G1 = G1::deserialize_with_mode(
        &*[
            &convert_endianness::<32, 64>(proof.try_into().unwrap()),
            &[0u8][..],
        ]
        .concat(),
        Compress::No,
    ...
    proof_a
        ...
        .serialize_with_mode(&mut proof_a_neg[32..], Compress::No)
        .unwrap();
    convert_endianness::<32, 64>(&proof_a_neg[..64].try_into().unwrap())
}
...
fn write_to_file(filename: &str, proof_a: &[u8], proof_b: &[u8], proof_c: &[u8]) {
    let mut file = File::create(filename).expect("Failed to create file");
    file.write_all(proof_a).expect("Failed to write proof_a");
    file.write_all(proof_b).expect("Failed to write proof_b");
    file.write_all(proof_c).expect("Failed to write proof_c");
}

fn write_proof_to_file(filename: &str, proof: &[u8]) {
    let mut file = File::create(filename).expect("Failed to create file");
