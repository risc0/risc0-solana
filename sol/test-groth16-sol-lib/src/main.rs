...
mod r0_tests {
    ...
    #[derive(Debug, Deserialize)]
    struct PublicInputs(Vec<String>);

    fn parse_verifying_key(file_path: &str) -> VerifyingKey {
    ...
    fn parse_proof(file_path: &str) -> Proof {
        let mut file = File::open(file_path).expect("Unable to open file");
        let mut contents = String::new();
        file.read_to_string(&mut contents)
            .expect("Unable to read file");
    ...
    fn parse_public_inputs(file_path: &str) -> PublicInputs {
        let mut file = File::open(file_path).expect("Unable to open file");
        let mut contents = String::new();
        file.read_to_string(&mut contents)
            .expect("Unable to read file");
    ...
    fn convert_to_vk(vk: VerifyingKey) -> Groth16Verifyingkey<'static> {
        let vk_alpha_g1: [u8; 64] = vk
            .vk_alpha_1
            .iter()
            .map(|s| str_to_u8_array::<32>(s))
            .flatten()
            .collect::<Vec<u8>>()
            .try_into()
            .unwrap();

        let vk_beta_g2: [u8; 128] = vk
            .vk_beta_2
            .iter()
            .flat_map(|arr| arr.iter().map(|s| str_to_u8_array::<32>(s)))
            .flatten()
            .collect::<Vec<u8>>()
            .try_into()
            .unwrap();

        let vk_gamme_g2: [u8; 128] = vk
            .vk_gamma_2
            .iter()
            .flat_map(|arr| arr.iter().map(|s| str_to_u8_array::<32>(s)))
            .flatten()
        ...
        let vk_ic = Box::leak(Box::new(vk_ic_data));
    ...
    fn str_to_u8_array<const N: usize>(s: &str) -> [u8; N] {
        let mut bytes = [0u8; N];
        let big_uint = BigUint::from_str_radix(s, 10).expect("Invalid number string");
        let byte_vec = big_uint.to_bytes_be();
        let start = N - byte_vec.len();
        bytes[start..].copy_from_slice(&byte_vec);
        bytes
    }

    #[test]
    fn proof_verification_should_succeed() {
        let vk = parse_verifying_key("data/verification_key.json");
        let proof = parse_proof("data/proof.json");
        let public_inputs: PublicInputs = parse_public_inputs("data/public.json");

        // TODO: fix me
        let vk_struct = convert_to_vk(vk);
        ...
        // let (compressed_proof_a, compressed_proof_b, compressed_proof_c) =
        //     proof.to_compressed_arrays();
...
mod light_protocol_tests {
    ...
    pub const LP_VERIFYING_KEY: Groth16Verifyingkey = Groth16Verifyingkey {
        nr_pubinputs: 10,
    ...
    pub const LP_PUBLIC_INPUTS: [[u8; 32]; 9] = [
        [
    ...
    fn proof_verification_should_succeed() {
        let proof_a: G1 = G1::deserialize_with_mode(
            &*[&change_endianness(&LP_PROOF[0..64]), &[0u8][..]].concat(),
            Compress::No,
            Validate::Yes,
        )
        .unwrap();
        ...
        let proof_b = LP_PROOF[64..192].try_into().unwrap();
        let proof_c = LP_PROOF[192..256].try_into().unwrap();
    ...
    fn proof_verification_with_compressed_inputs_should_succeed() {
        ...
        for input in LP_PUBLIC_INPUTS.chunks(32) {
            public_inputs_vec.push(input);
        }
        let compressed_proof_a = compress_g1_be(&LP_PROOF[0..64].try_into().unwrap());
        let compressed_proof_b = compress_g2_be(&LP_PROOF[64..192].try_into().unwrap());
        let compressed_proof_c = compress_g1_be(&LP_PROOF[192..].try_into().unwrap());
    ...
    fn wrong_proof_verification_should_not_succeed() {
        let proof_a = LP_PROOF[0..64].try_into().unwrap();
        let proof_b = LP_PROOF[64..192].try_into().unwrap();
        let proof_c = LP_PROOF[192..256].try_into().unwrap();
        let mut verifier = Groth16Verifier::new(
